...
- ~~**W→W on the ASB emulator**~~ — **now delivered** by the WolverineFx **4.12.2 → 6.4.3** upgrade in [issue #28](https://github.com/Pieter-1337/DDD/issues/28) (this requirement's #24 baseline was deliberately RabbitMQ-only). WolverineFx 6's `AzureServiceBusTransport.ManagementConnectionString` is fed from the AppHost's existing `messaging-admin` string on the emulator path, so `AutoProvision` reaches the port-5300 management plane; native W→W is now verified end-to-end on the emulator. The 4→6 jump required referencing `WolverineFx.RuntimeCompilation` (GH-2876) and setting `ServiceLocationPolicy.AllowedButWarn` — see [ADR-0003](../adr/0003-native-wolverine-flow-and-framework-alignment.md) and the [broker × framework matrix](../phase-5-event-driven/09-broker-framework-matrix.md). (Originally out of scope here: 4.12.2's `AzureServiceBusTransport` had no `ManagementConnectionString`, confirmed by a 2026-06-04 build probe.)
