...
- **The AppHost fans out one value.** `Aspire.AppHost` exposes a single `messaging-framework` parameter (default `MassTransit`) and injects it into both services via `WithEnvironment("MessagingFramework", …)`, exactly as it already centralises the broker resource. One knob, both align, cannot misconfigure locally. This does **not** violate ADR-0001's "one mechanism" principle: services still read the same `MessagingFramework` config key they read in prod — the AppHost is merely one config *source* (like user-secrets) supplying an aligned value for local dev.
