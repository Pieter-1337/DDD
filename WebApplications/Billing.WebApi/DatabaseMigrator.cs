...
public class DatabaseMigrator : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            _logger.LogInformation("Applying pending EF Core migrations for {Context}…", nameof(BillingDbContext));
            await context.Database.MigrateAsync(timeoutCts.Token);
            _logger.LogInformation("{Context} schema is up to date.", nameof(BillingDbContext));
        }
        ...
        {
            _logger.LogError(ex, "Failed to apply migrations for {Context}.", nameof(BillingDbContext));
            throw new InvalidOperationException(
                "Database migration failed for the Billing context. Verify SQL Server/LocalDB is running and that " +
